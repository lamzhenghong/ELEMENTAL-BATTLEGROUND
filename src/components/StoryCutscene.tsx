...
export default function StoryCutscene({ slides, onComplete, backgroundGradient }: StoryCutsceneProps) {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-slate-950 select-none ${isShaking ? 'animate-shake' : ''}`}>
      ...
      <div className="relative w-full max-w-4xl h-full flex flex-col justify-center items-center px-6 pb-40">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className={`w-32 h-32 md:w-40 md:h-40 rounded-full flex items-center justify-center text-slate-950 text-5xl md:text-6xl font-black bg-gradient-to-tr from-amber-400 to-indigo-500 shadow-2xl border-4 border-slate-900`}>
              {currentLine.speaker.charAt(0)}
            </div>
            
            {/* Subdued name indicator */}
            <div className="bg-black/55 border border-white/5 px-4 py-1.5 rounded-full text-slate-300 font-mono text-[10px] md:text-xs uppercase tracking-widest">
